...
class RecordService {
  ...
  async getRecord(recordId) {
    try {
      return await Record.findById(recordId).where("deletedAt").equals(null);
    } catch (error) {
      return null;
    }
  }
  ...
  async listRecords(page = 1, pageSize = 20, filters = {}) {
    try {
      ...
      let query = Record.find({ deletedAt: null });
      ...
      // Get total count
      const total = await Record.countDocuments({ deletedAt: null });
  ...
  async listRecordsByOwner(ownerEmail, page = 1, pageSize = 20, filters = {}) {
    try {
      ...
      // Build query
      let query = Record.find({ ownerEmail, deletedAt: null });
